...
import { usePendingRevenuesSecure } from "@/hooks/usePendingRevenuesSecure";
...
export const PendingRevenuesCardSecure = ({ onNavigate }: PendingRevenuesCardProps) => {
  const { pendingRevenues, loading } = usePendingRevenuesSecure();
    ...
    >
      ...
      <CardContent>
        ...
        <div className="flex items-center justify-between mt-2">
          <p className="text-xs text-muted-foreground">
            {activePendingRevenues.length} receita{activePendingRevenues.length !== 1 ? 's' : ''} da empresa
          </p>
          {urgentCount > 0 && (
